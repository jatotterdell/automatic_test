...
model {
  ...
  for (i in 1:N) {
    if(control[i] == 1) {
      // beta_ctr is the overall control log odds of vaccination on time.
      eta[i] += beta_ctr;
    }
    if(control[i] == 0) {
      // beta_trt is the overall control log odds of vaccination on time across
  ...
  target += student_t_lpdf(sd_1 | 3, 0, ran_sd) - 1 * student_t_lccdf(0 | 3, 0, ran_sd)
          + student_t_lpdf(sd_2 | 3, 0, ran_sd) - 1 * student_t_lccdf(0 | 3, 0, ran_sd)
          + student_t_lpdf(sd_3 | 3, 0, ran_sd) - 1 * student_t_lccdf(0 | 3, 0, ran_sd)
          + normal_lpdf(gamma1_raw | 0, 1)
          + normal_lpdf(gamma2_raw | 0, 1)
          + normal_lpdf(gamma3_raw | 0, 1);
...
generated quantities {
  ...
  // that is, equivalent to working through a matrix row-wise
  for(i in 1:L1) {
    for(j in 1:L2) {
